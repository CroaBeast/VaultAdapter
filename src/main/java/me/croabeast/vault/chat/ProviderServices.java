...
final class ProviderServices {
    ...
    private static final Map<ChatProvider, ServiceRegistration> REGISTRATIONS = new IdentityHashMap<>();
    ...
    static ChatProvider detect(@Nullable ChatProvider self) {
        ...
        ChatProvider fromLuckPerms = detectLuckPerms();
        if (fromLuckPerms != null) return fromLuckPerms;

        ChatProvider fromVault2 = detectVault2();
        if (fromVault2 != null) return fromVault2;

        ChatProvider fromVault = detectVault();
    ...
    private static boolean unregister0(ChatProvider backend) {
    ...
    @Nullable
    private static ChatProvider detectLuckPerms() {
        try {
    ...
    @Nullable
    private static ChatProvider detectVault2() {
        try {
            Class<?> type = Class.forName("net.milkbowl.vault2.chat.Chat");
            RegisteredServiceProvider<?> registration = Bukkit.getServicesManager().getRegistration(type);
            if (registration == null) return null;
    ...
    @Nullable
    private static ChatProvider detectVault() {
        try {
            Class<?> type = Class.forName("net.milkbowl.vault.chat.Chat");
            RegisteredServiceProvider<?> registration = Bukkit.getServicesManager().getRegistration(type);
            if (registration == null) return null;
    ...
    private static ServiceRegistration registerVault2(Plugin plugin, ChatProvider backend) {
        try {
            Class<?> serviceType = Class.forName("net.milkbowl.vault2.chat.Chat");
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    private static ServiceRegistration registerVault(Plugin plugin, ChatProvider backend) {
        try {
            Class<?> serviceType = Class.forName("net.milkbowl.vault.chat.Chat");
